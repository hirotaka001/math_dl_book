...
class Indexes():
    ...
    def next_index(self):
        ...
        # batchサイズより作業用indexesが小さい場合はindexes再生成
...
# 結果確認　その１
# 損失関数値と精度の確認
print('初期状態: 損失関数:%f 精度:%f' % (history1[0, 1], history1[-1, 2]))
...
plt.yticks(size=14)
plt.grid(lw=2)
plt.savefig('learning_curve_loss1.png')
...
plt.yticks(size=14)
plt.grid(lw=2)
plt.savefig('learning_curve_accuracy1.png')
...
# 結果確認 その2
# 損失関数値と精度の確認
print('初期状態: 損失関数:%f 精度:%f' % (history2[0, 1], history2[0, 2]))
print('最終状態: 損失関数:%f 精度:%f' % (history2[-1, 1], history2[-1, 2]))
...
plt.yticks(size=14)
plt.grid(lw=2)
plt.savefig('learning_curve_loss2.png')
...
plt.yticks(size=14)
plt.grid(lw=2)
plt.savefig('learning_curve_accuracy2.png')
...
plt.xlim(-4, 4)
plt.ylim(-4, 4)
plt.xlabel(r'$x$', fontdict=14)
plt.ylabel(r'$y$', fontdict=14)
plt.grid(lw=2)
plt.plot(xx, ReLU(xx), c='b', label='ReLU', linestyle='-', lw=3)
plt.plot(xx, step(xx), c='k', lagel='step', linestyle='-', lw=3)
plt.xticks(size=14)
plt.yticks(size=14)
plt.legend(fontsize=14)
plt.savefig('ReLU_step.png')
...
while epoch < nb_epoch:
    ...
    if next_flag:                       # 1epoch終了後の処理
        score, loss = evaluate2(x_test, y_test, y_test_one, V, W)
...
plt.yticks(size=14)
plt.grid(lw=2)
plt.savefig('learning_curve_loss3.png')
...
plt.yticks(size=14)
plt.grid(lw=2)
plt.savefig('learning_curve_accuracy3.png')
...
while epoch < nb_epoch:
    ...
    if next_flag:                           # 1epoch 終了後の処理
        score, loss = evaluate3(x_test, y_test, y_test_one, U, V, W)
